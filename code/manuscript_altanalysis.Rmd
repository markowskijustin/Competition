...
for (k in 1:28){
  ...
  model <- feols(Outcome ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + MHC + HO + PH + UrbanRuralFlag| ID + Year | I((10000 - ClinicHHI)/1000) ~ value, cluster = "ID", data = df.temptemptemp)
...
summary(feols(df.temptemp %>% mutate(Tx = ifelse(Tx > 1, 1, 0)) %>% filter(!is.na(Tx)) %>% filter(value_min <= 2), 
              Qcomp ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + UrbanRuralFlag | ID + Year | I((10000 - ClinicHHI)/100) ~ Tx, cluster = "ID"))
...
for (k in 1:22){
  ...
  model <-  did2s(
      ...
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + UrbanRuralFlag | ID + Year , #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None | ID + Year , 
        second_stage = ~ i(Tx, ref = FALSE),
        cluster_var = "ID",
      verbose = TRUE
    )
...
did2s(
      data = df.temptemp %>% mutate(Tx = ifelse(Tx > 1, 1, 0)) %>% filter(!is.na(Tx)) %>% mutate(Mix = Pct.COM * Total / 100) %>% filter(Year < 2020 ) %>% mutate(Urban = ifelse(UrbanRuralFlag == "Urban", 1, 0)) %>% filter(!is.na(Urban)), 
      yname = "Staff", 
      treatment = "Tx",
        ...
        second_stage = ~ i(Tx, ref = FALSE) + I(Tx * value),
        cluster_var = "ID",
      verbose = TRUE
    )
...
es.direct <- did2s(
  data = df.temptemp %>% mutate(Tx = ifelse(Tx > 1, 1, 0)) %>% filter(!is.na(Tx)) %>% mutate(Mix = Pct.COM * Total / 100) %>% mutate(Urban = ifelse(UrbanRuralFlag == "Urban", 1, 0))  %>% mutate(rel_year = ifelse(Att == 0, Inf, Year - Att)) , 
  yname = "Qcomp", 
  treatment = "Tx",
  first_stage = ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless | ID + Year, 
...
cbind(YearRel = gsub("rel_year::", "", rownames(es.direct$coeftable)), 
      cbind(Group = "Direct",es.direct$coeftable)) %>% 
  data.frame() %>% 
...
df.z.temp <- df.z %>% mutate(Tx = ifelse(value - 1 >= 1, 1, value - 1)) %>% filter(!is.na(Tx)) %>% mutate(DirectlyTreated = ifelse(Att.z >= 2010, 1, 0)) %>% 
  ...
  select(NumberofPatients, Pct.MCD:Pct.None, Total.Hours, Sites, Mean.Hours, Perminance, Schedule, Calendar, Tx, DirectlyTreated, ID, Year, Zip, Att, `BHCMIS ID`, UrbanRuralFlag) 
...
es.direct <- did2s(
  data = df.z.temp2 %>% mutate(rel_year = ifelse(Att == 0, Inf, Year - Att)) %>% filter(Att == 0 | DirectlyTreated == 1), 
  yname = "Sites", 
  treatment = "Tx",
...
es.indirect <- did2s(
  data = df.z.temp2 %>% mutate(rel_year = ifelse(Att == 0, Inf, Year - Att)) %>% filter(Att == 0 | DirectlyTreated == 0), 
  yname = "Sites", 
  treatment = "Tx",
  first_stage = ~ 0 | ID + Year, 
    second_stage = ~ i(rel_year, ref = c(-1, Inf)) ,
...
for (k in 1:28){
  ...
  #model <- feols(Outcome ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless   | I((10000 - ClinicHHI)/1000) ~ Tx + factor(Year) + factor(ID) , cluster = "ID", data = df.temptemptemp)
  #coeffs <- rbind(coeffs, model$coeftable[2,])
  model <- feols(Outcome ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + UrbanRuralFlag | Year | I((10000 - ClinicHHI)/1000) ~ Z, cluster = "ID", data = df.temptemptemp)
  coeffs <- rbind(coeffs, model$coeftable[1,])
