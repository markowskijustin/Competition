...
length(unique(uds.covariates$BHCMISID))
...
length(unique(df.comp[df.comp$Year %in% included.years,]$BHCMISID))


df.comp %>% 
    filter(Year == 2022) %>% 
    mutate(Missing = ifelse(is.na(ngla_net_gain_loss), "Missing", "Have")) %>% 
    ungroup() %>% 
    select(Missing, Total, Pct.NHA:Pct.COM) %>% 
...
length(unique(df.temptemp$`BHCMIS ID`))
...
df.temptemp <- df.temptemp %>%
  ...
  mutate(ACA = ifelse((STATE %in% c('MI','NH') & Year >= 2014) |
                    ...
                    (STATE %in% c('OK','MO','AR') & Year >= 2021) |
                    (STATE %in% c('SD','NC') & Year >= 2023) | 
                    (STATE %in% c('AZ','CA','CO','CT','DE','DC','HI','IL','IA','KY','MD','MA','MN','NV','NJ','NM','NY','ND','OH','OR','PA','RI','VT','VA','WA') & Year >= 2014), 1, 0))
...
summary(df.temptemp[df.temptemp$Year == 2022 & df.temptemp$Closed == 0,])

df.temptemp[df.temptemp$Closed == 0,]


# 
# fullfins <- ufins2 %>%
#   mutate(months_of_cash = as.numeric(months_of_cash),
#                ngla_prog_serv_revenue = as.numeric(ngla_prog_serv_revenue),
#                quick_ratio = as.numeric(quick_ratio),
#                Liquidity = (Assets - Liabilities) / Assets,
#                Profitability = ngla_revenue / ngla_expense * 100, #Assets,
...
#          NetWorth = ifelse(is.infinite(NetWorth), NA, NetWorth),
#          Altman = (0.0656 * Liquidity + 0.0326 * quick_ratio + 0.0672 * Efficiency + 0.0105 * NetWorth) * 10) %>%
#   select(BHCMIS.ID, form_year, Altman, quick_ratio, months_of_cash, Profitability, LiabPctAssets, Staff, ngla_net_gain_loss )
# 
# 
# 
# 
# df.temptemp <- df.temptemp %>%
#   left_join(fullfins %>% unique(), by = c("BHCMIS ID" = "BHCMIS.ID","Year" = "form_year")) %>%
#   unique() %>%
#   mutate(Altman.x = ifelse(is.na(Altman.x), Altman.y, Altman.x),
#          quick_ratio.x = ifelse(is.na(quick_ratio.x), quick_ratio.y, quick_ratio.x),
#          months_of_cash.x = ifelse(is.na(months_of_cash.x), months_of_cash.y, months_of_cash.x),
#          Profitability.x = ifelse(is.na(Profitability.x), Profitability.y, Profitability.x),
#          LiabPctAssets.x = ifelse(is.na(LiabPctAssets.x), LiabPctAssets.y, LiabPctAssets.x),
#          Staff.x = ifelse(is.na(Staff.x), Staff.y, Staff.x),
#          ngla_net_gain_loss.x = ifelse(is.na(ngla_net_gain_loss.x), ngla_net_gain_loss.y, ngla_net_gain_loss.x)) %>%
#   rename(Altman = Altman.x,
#          quick_ratio = quick_ratio.x,
#          months_of_cash = months_of_cash.x,
#          Profitability = Profitability.x,
#          LiabPctAssets = LiabPctAssets.x,
#          Staff = Staff.x,
#          ngla_net_gain_loss = ngla_net_gain_loss.x) %>%
#   unique()
...
df.temptemp %>% 
  filter(Year == 2022) %>% 
  mutate(Missing = ifelse(is.na(Qcomp), "Missing", "Have")) %>% 
  select(Missing, Total, Pct.NHA:Staff) %>% 
  #filter(!complete.cases(.)) %>% 
  tbl_summary(by = Missing, 
                statistic = list(all_continuous() ~ "{mean} ({sd})"),
missing = "no", digits = list(everything() ~ c(2))) %>% 
...
df.temptemp %>% 
  filter(Year == 2022) %>% 
  mutate(Missing = ifelse(is.na(Altman), "Missing", "Have")) %>% 
  select(Missing, Total, Pct.NHA:Staff) %>% 
  #filter(!complete.cases(.)) %>% 
  tbl_summary(by = Missing, 
                statistic = list(all_continuous() ~ "{mean} ({sd})"),
...
## Balance by 990 vs non990 data CHCs
```{r}
...
df.temptemp <- df.temptemp %>%
  left_join(fullfins %>% unique() %>%
  filter(!(`BHCMIS.ID` == "03E00494" & form_year == 2010) & 
           !(`BHCMIS.ID` == "03E01039" & form_year ==	2015)& 
           !(`BHCMIS.ID` == "05E00445" & form_year ==	2015)& 
           !(`BHCMIS.ID` == "05E00499" & form_year ==	2013)& 
           
           !(`BHCMIS.ID` == "06E00500" & form_year ==	2016)& 
           !(`BHCMIS.ID` == "07E00486" & form_year ==	2014)& 
           !(`BHCMIS.ID` == "07E00514" & form_year ==	2014)& 
           !(`BHCMIS.ID` == "10E00136" & form_year ==	2013)), by = c("BHCMIS ID" = "BHCMIS.ID","Year" = "form_year"))  %>% 
  unique() %>%
  mutate(Altman.x = ifelse(is.na(Altman.x), Altman.y, Altman.x),
         quick_ratio.x = ifelse(is.na(quick_ratio.x), quick_ratio.y, quick_ratio.x),
         months_of_cash.x = ifelse(is.na(months_of_cash.x), months_of_cash.y, months_of_cash.x),
...
df.temptemp %>% 
  filter(Year == 2010) %>% 
  select(Altman, Pct.NHA:Pct.Homeless, Pct.MCD:Pct.None) %>% 
  mutate(Missing = ifelse(is.na(Altman), "Missing", "Have")) %>% 
  #filter(!complete.cases(.)) %>% 
  tbl_summary(by = Missing, 
                statistic = list(all_continuous() ~ "{mean} ({sd})"),
...
# 
# fullfins %>% 
#     filter(`BHCMIS.ID` %in% names(table(df.temptemp$`BHCMIS ID`)[table(df.temptemp$`BHCMIS ID`) != 15]))
# 
# df.temptemp %>% 
#   filter(`BHCMIS ID` %in% names(table(df.temptemp$`BHCMIS ID`)[table(df.temptemp$`BHCMIS ID`) != 15]))
...
```{r}
uds.6b.2022 %>% 
  filter(BHCMISID == "01E01088")

uds.quality.6b %>% 
  filter(BHCMISID == "01E01088")

df.temptemp %>% 
  filter(`BHCMIS ID` == "01E01088")
```
...
plot0 <- p %>% 
  arrange(CompetitorsInArea) %>% 
  ggplot(aes(x = Year, y = Count, group = CompetitorsInArea, linetype = CompetitorsInArea)) + ylab("CHCs") + 
  #geom_point(size = 1.1) + 
  geom_line(size = 1.1)  + theme_tufte_revised() + ggtitle("CHCs by Number of Colocated CHCs") + 
  #geom_dl(label=c(rep("0 Rivals", length(unique(p$Year))), rep("1 Rival", length(unique(p$Year))), rep("2 Rivals", length(unique(p$Year))), rep("3+ Rivals", length(unique(p$Year)))), 
...
plot1 <- uds.sites.full[!is.na(uds.sites.full$Zip),] %>% 
  ...
  summarise(Zips = sum(Zips)) %>% 
  #filter(Year >= 2010) %>% 
  ggplot(aes(x = Year, y = Zips)) + 
  #geom_point(size = 1.1) + 
  geom_line(size = 1.1, aes(linetype = Counts.f)) + theme_tufte_revised() +
  ggtitle("Zip Codes by Number of CHC Sites") + 
  #geom_dl(label=c(rep("1 Site", length(unique(p$Year))), rep("2+ Sites", length(unique(p$Year)))), method = list("last.points", hjust = 1, vjust = -1, cex = 1.5), inherit.aes=T ) + 
  #guides(linetype = "none") + 
  ylim(0, 4000) +theme(axis.text=element_text(size=12),
        axis.title=element_text(size=12,face="bold")) + 
  scale_linetype_manual(values=c(7, 8)) + 
  guides(linetype = guide_legend(title = "# CHC Sites by\nZip Code"))


p.desc <- ggarrange(plot1, plot0, ncol = 1) 

p.desc <- annotate_figure(p.desc, top = text_grob("CHC Growth and Density by Zip and Clinic, UDS 1996 to 2023", face = "bold", size = 14))
...
#ggsave(filename = "Figure1.pdf", path = "/Users/markow/Dropbox/Writing/CHC Competitive Shocks/Revision 1", plot = p.desc, width = 7, height = 10)
```
...
for (k in 1:28){
  ...
  model <- feols(Outcome ~ Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless  | ID + Year | I((10000 - ClinicHHI)/1000) ~ Tx, cluster = "ID", data = df.temptemptemp)
...
## Final DID Event Studies and Alt Estimate Models (Sun-Abraham, TWFE)


```{r}
  
  
es.data <- data.frame(estimator = 0, term = 0, estimate = 0, std.error = 0, Measure = 0)
...
for (k in 1:23){
  ...
  model <-  event_study(data = df.temptemptemp,
  yname = "Outcome", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + UrbanRuralFlag) %>%
  filter(abs(term) <= 7) 
  

  es.data <- rbind(es.data, data.frame(model, Measure = colnames(df.temptemp)[(16+k)]))
...
ess <- es.data[-1,] %>% 
  ...
  #filter(estimator %notin% c("Callaway and Sant'Anna (2020)","Roth and Sant'Anna (2021)")) %>% 
  filter(estimator == "Gardner (2021)") %>% 
  
  filter(Measure %in% c('Total','Staff','Altman','Pct.MCD','Pct.None','Complexity')) %>% 
  mutate(Group = ifelse(Measure %in% c('Total','Staff','logPatsPerStaff','Qcomp'), "Operational Metrics", 
                        ifelse(Measure %in% c('Altman','LiabPctAssets','months_of_cash'), 'Financial Metrics', 'Selection Metrics'))) %>% 
  mutate(Measure = ifelse(Measure == "Altman", "Financial resilience, Altman’s Z Score", 
                   ifelse(Measure == "Staff", "CHC Staff, No.",
                   ifelse(Measure == "Total", "Total Patients Served, No.", 
                   ifelse(Measure == "Complexity", "Average prevalence of select chronic conditions, %", 
                   ifelse(Measure == "Pct.MCD", "Payer Mix - % Medicaid", 
                   ifelse(Measure == "Pct.None", "Payer Mix - % Uninsured", Measure))))))) %>%
  rename(Year = term) %>% 
  ggplot(aes(x = Year, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9), size = 3) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.25, position = position_dodge(0.9), size = 0.8)  +
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ #theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) +
...
ess + theme(legend.position="none")
ess + theme(legend.position="bottom")

ggsave(filename = "Figure2.pdf", path = "/Users/markow/Dropbox/Writing/CHC Competitive Shocks/Revision 1", plot = ess + theme(legend.position="none"), width = 8, height = 10)
...
df.temptemp %>% 
  ...
  group_by(Post, Treat) %>% 
  select(Closed, Complexity) %>% 
  summarise(mean.nas(Closed), n(), mean.nas(Complexity))
...
## Stratified figure instead of table - optional
```{r}

strat.temp <- coeffs.did.strat
strat.temp[3,] <- strat.temp[3,] / 1000
strat.temp[23,] <- strat.temp[23,] / 10
strat.temp[18,] <- strat.temp[18,] / 100

#strat.temp[12,] <- strat.temp[12,] / 5.5 * 100
...
strat.temp <- (strat.temp/(df.temptemp %>% 
  mutate(Post = ifelse(year.rel >= 0, 1, 0)) %>% 
  filter(!is.na(value) & value_min > 1) %>% 
  group_by(Post, Treat) %>% 
  select(Qcomp:Closed) %>% 
  summarise_all(mean.nas) %>% 
  filter(Post == 0 & Treat == "Treated") %>% 
  t())[-c(2)] %>% 
  as.numeric() * 100)[-24,] %>% 
  rbind(coeffs.did.strat[24,])
...
strat.temp$Outcome <- rep(c("Composite Quality of Care", "Total Patients Served","Financial resilience,\nAltman’s Z Score", "Patient-to-Staff Ratio","Average prevalence of\nselect chronic conditions","Intensity", "Medicaid","Uninsured","CostPerPat","Quick Ratio","Months of Cash Available", "Revenue-to-Expenses","Liabilities-to-Assets","Staff","Closure, Acquisition or\nloss of FQHC status"), 2)



strat.temp <- strat.temp %>% 
  select(Outcome) %>% 
  cbind(data.frame(rbind(c(1.82, -0.4, 4.04),
      c(-448, -2513, 1617), 
      c(-0.08, -0.33, 0.16), 
      c(6.30, 0.4, 12.2), 
      c(-0.18, -0.71, 0.34), 
      ...
      c(-1.25, -2.87, 0.37), 
      c(5.36, 1.62, 9.11), 
      c(81.13, 46.12, 116.14), 
      c(1.67, 0.62, 2.73)))  * 100 / as.numeric((df.temptemp %>% 
  mutate(Post = ifelse(year.rel >= 0, 1, 0)) %>% 
  filter(!is.na(value) & value_min > 1) %>% 
  group_by(Post, Treat) %>% 
  select(Qcomp, Total, Altman, logPatsPerStaff, Complexity, Intensity, Pct.MCD, Pct.None, CostPerPat, quick_ratio, months_of_cash, Profitability, LiabPctAssets, Staff) %>% 
  summarise_all(mean.nas) %>% 
  filter(Post == 0 & Treat == "Treated") %>% 
  t() %>% 
    rbind(100))[-c(1,2),])
...
p <- strat.temp  %>% 
  filter(Outcome %notin% c("Intensity","CostPerPat","Months of Cash Available")) %>% 
...
p + theme(panel.spacing = unit(1, "lines"),  strip.text = element_text(face = "bold", size = 12))



colnames(df.temptemp)[17:39]



ggsave(filename = "Figure3.pdf", path = "/Users/markow/Dropbox/Writing/CHC Competitive Shocks/Revision 1", plot = p + theme(panel.spacing = unit(1, "lines"),  strip.text = element_text(face = "bold", size = 12)), width = 8, height = 10)
...
## Compared to pre-2020 estimates and Controlling for ACA
```{r}




coeffs.did.ACA <- c(0,0,0,0)
...
for (k in 1:23){
  ...
  model <-  did2s(
      ...
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + UrbanRuralFlag + ACA | ID + Year , #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None | ID + Year , 
        second_stage = ~ i(Tx, ref = FALSE),
        cluster_var = "ID",
      verbose = TRUE
    )
  ...
  coeffs.did.ACA <- rbind(coeffs.did.ACA, model$coeftable[1,])
  

}


core.did.estimates.ACA <- data.frame(cbind(colnames(df.temptemp)[17:39], (coeffs.did.ACA[-1,]))) %>% 
  #select(V1, Estimate, Pr...t.., Estimate.1, Pr...t...1) %>% 
  mutate(Estimate = round(as.numeric(Estimate), 2), `95% CI` = paste(round(as.numeric(Estimate) - 1.96 * as.numeric(`Std..Error`), 2), ", ", round(as.numeric(Estimate) + 1.96 * as.numeric(`Std..Error`), 2), sep = "")) %>% 
  select(V1, Estimate, `95% CI`, Pr...t..)

core.did.estimates.ACA <- core.did.estimates.ACA
#rownames(core.did.estimates) <- NULL
...
cbind(core.did.estimates[,c(1,2,4)], core.did.estimates.pre2020[,c(2,4)], core.did.estimates.ACA[,c(2,4)])
...
for (k in 1:23){
  ...
  df.temptemptemp <- df.temptemptemp %>% 
    mutate(Outcome = ifelse(Outcome <= outlier.vals[1], outlier.vals[1], 
                            ifelse(Outcome >= outlier.vals[2], outlier.vals[2], Outcome)))
  model <-  did2s(
      data = df.temptemptemp %>% inner_join(df.temptemptemp %>% group_by(`BHCMIS ID`) %>% summarise(ACA = max(ACA)) %>% filter(ACA == 1), by = c("BHCMIS ID")) %>% filter(Year < 2024),  
      #data = df.temptemptemp %>% filter(ACA == 0 & Year <= 2024), 
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + UrbanRuralFlag | ID + Year , #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None | ID + Year , 
        second_stage =~ i(Tx*ifelse(value_min== 1, 1, 0), ref = FALSE) + i(Tx*ifelse(value_min >= 2, 1, 0), ref = FALSE),
  ...
  coeffs.did.ACA <- rbind(coeffs.did.ACA, c(model$coeftable[1,], model$coeftable[2,]))
...
for (k in 1:23){
  ...
  df.temptemptemp <- df.temptemptemp %>% 
    mutate(Outcome = ifelse(Outcome <= outlier.vals[1], outlier.vals[1], 
                            ifelse(Outcome >= outlier.vals[2], outlier.vals[2], Outcome)))
  model <-  did2s(
      data = df.temptemptemp %>% inner_join(df.temptemptemp %>% group_by(`BHCMIS ID`) %>% summarise(ACA = max(ACA)) %>% filter(ACA == 0), by = c("BHCMIS ID")) %>% filter(Year < 2024),  
      #data = df.temptemptemp %>% filter(ACA == 0 & Year <= 2024), 
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + UrbanRuralFlag | ID + Year , #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None | ID + Year , 
  ...
  coeffs.did.nonACA <- rbind(coeffs.did.nonACA, c(model$coeftable[1,], model$coeftable[2,]))
  

}


core.did.estimates.nonACA <- data.frame(cbind(colnames(df.temptemp)[17:39], (coeffs.did.nonACA[-1,]))) %>% 
  #select(V1, Estimate, Pr...t.., Estimate.1, Pr...t...1) %>% 
  mutate(Estimate = round(as.numeric(Estimate), 2), `95% CI` = paste(round(as.numeric(Estimate) - 1.96 * as.numeric(`Std..Error`), 2), ", ", round(as.numeric(Estimate) + 1.96 * as.numeric(`Std..Error`), 2), sep = "")) %>% 
  mutate(Estimate.1 = round(as.numeric(Estimate.1), 2), `95% CI.1` = paste(round(as.numeric(Estimate.1) - 1.96 * as.numeric(`Std..Error.1`), 2), ", ", round(as.numeric(Estimate.1) + 1.96 * as.numeric(`Std..Error.1`), 2), sep = "")) %>% 
  select(V1, Estimate, `95% CI`, Pr...t.., Estimate.1, `95% CI.1`, `Pr...t...1`)

core.did.estimates.nonACA <- core.did.estimates.nonACA
#rownames(core.did.estimates) <- NULL
...
cbind(core.did.estimates.ACA, core.did.estimates.nonACA)


df.temptemptemp %>% inner_join(df.temptemptemp %>% group_by(`BHCMIS ID`) %>% summarise(ACA = max(ACA)), by = c("BHCMIS ID")) %>% 
  mutate(value = ifelse(value_min == 1, 0, 1)) %>% 
  ungroup() %>% 
  select(ACA.y, value) %>% 
  group_by(ACA.y, value) %>% 
  summarise(n())
...
coeffs.did.missingButBaseline <- c(0,0,0,0)

for (k in 1:22){

  #df.temptemptemp <- df.temptemp %>% mutate(Tx = ifelse(Tx > 1, 1, 0)) %>% filter(!is.na(Tx)) #%>% group_by(ID) %>% mutate(EarlyHHI = first(ClinicHHI, na_rm = TRUE)) %>% filter(EarlyHHI < 9000)
  df.temptemptemp <- df.temptemp %>% mutate(Tx = ifelse(Year >= Att & Att %notin% c(0, 2009, 2010), 1, 0)) %>% filter(!is.na(Tx)) %>% select(ClinicHHI:ACA, Treat:year.rel) %>% ungroup() %>% # filter(!is.na(Altman)) 
    inner_join(df.temptemp %>% filter(Year == 2010) %>% ungroup() %>% mutate(Missing = ifelse(is.na(Altman), "Missing", "Have")) %>% select(`BHCMIS ID`, Missing), by = c("BHCMIS ID")) %>% filter(Missing == "Have") %>% 
    filter(!complete.cases(.)) 
  colnames(df.temptemptemp)[(16+k)] <- "Outcome"
  # Winsorized
  outlier.vals <- quantile(df.temptemptemp$Outcome, p = c(0.01, 0.99), na.rm = TRUE)
  ...
  coeffs.did.missingButBaseline <- rbind(coeffs.did.missingButBaseline, model$coeftable[1,])
  

}


core.did.estimates.missingBugBaseline <- data.frame(cbind(colnames(df.temptemp)[17:39], (coeffs.did.missingButBaseline[-1,]))) %>% 
  #select(V1, Estimate, Pr...t.., Estimate.1, Pr...t...1) %>% 
  mutate(Estimate = round(as.numeric(Estimate), 2), `95% CI` = paste(round(as.numeric(Estimate) - 1.96 * as.numeric(`Std..Error`), 2), ", ", round(as.numeric(Estimate) + 1.96 * as.numeric(`Std..Error`), 2), sep = "")) %>% 
  select(V1, Estimate, `95% CI`, Pr...t..)

nrow(df.temptemptemp)


coeffs.did.notmissing <- c(0,0,0,0)
...
for (k in 1:22){
 ...
 if (sum(is.na(df.temptemptemp$Outcome)) > 2500) {
   coeffs.did.notmissing <- rbind(coeffs.did.notmissing, c(NA, NA, NA, NA))  
  ...
  coeffs.did.notmissing <- rbind(coeffs.did.notmissing, model$coeftable[1,])  
...
core.did.estimates.notmissing <- data.frame(cbind(colnames(df.temptemp)[17:39], (coeffs.did.notmissing[-1,]))) %>% 
  #select(V1, Estimate, Pr...t.., Estimate.1, Pr...t...1) %>% 
  mutate(Estimate = round(as.numeric(Estimate), 2), `95% CI` = paste(round(as.numeric(Estimate) - 1.96 * as.numeric(`Std..Error`), 2), ", ", round(as.numeric(Estimate) + 1.96 * as.numeric(`Std..Error`), 2), sep = "")) %>% 
  select(V1, Estimate, `95% CI`, Pr...t..)
...
coeffs.did.missing <- c(0,0,0,0)

for (k in 1:22){

  #df.temptemptemp <- df.temptemp %>% mutate(Tx = ifelse(Tx > 1, 1, 0)) %>% filter(!is.na(Tx)) #%>% group_by(ID) %>% mutate(EarlyHHI = first(ClinicHHI, na_rm = TRUE)) %>% filter(EarlyHHI < 9000)
  df.temptemptemp <- df.temptemp %>% mutate(Tx = ifelse(Year >= Att & Att %notin% c(0, 2009, 2010), 1, 0)) %>% filter(!is.na(Tx)) %>% select(ClinicHHI:ACA, Treat:year.rel) %>% ungroup() %>% # filter(!is.na(Altman)) 
    inner_join(df.temptemp %>% filter(Year == 2010) %>% ungroup() %>% mutate(Missing = ifelse(is.na(Altman), "Missing", "Have")) %>% select(`BHCMIS ID`, Missing), by = c("BHCMIS ID")) %>% filter(Missing == "Missing") 
 ...
 if (sum(is.na(df.temptemptemp$Outcome)) > 2500) {
   coeffs.did.missing <- rbind(coeffs.did.missing, c(NA, NA, NA, NA))  
  ...
  coeffs.did.missing <- rbind(coeffs.did.missing, model$coeftable[1,])  
...
core.did.estimates.missing <- data.frame(cbind(colnames(df.temptemp)[17:39], (coeffs.did.missing[-1,]))) %>% 
  #select(V1, Estimate, Pr...t.., Estimate.1, Pr...t...1) %>% 
  mutate(Estimate = round(as.numeric(Estimate), 2), `95% CI` = paste(round(as.numeric(Estimate) - 1.96 * as.numeric(`Std..Error`), 2), ", ", round(as.numeric(Estimate) + 1.96 * as.numeric(`Std..Error`), 2), sep = "")) %>% 
  select(V1, Estimate, `95% CI`, Pr...t..)
...
cbind(core.did.estimates.missingBugBaseline[,c(1,2,3, 4)], core.did.estimates.notmissing[,c(2,3, 4)], core.did.estimates.missing[,c(2,3,4)])
...
for (k in 1:9){
  ...
  model <-  did2s(
      ...
      yname = "Outcome", 
      treatment = "Tx",
        #first_stage = ~ 1 | ID + Year ,
        first_stage = ~ TotalPop + PctMedicaid + PctUninsured + PctPoverty + PctBlack + PctHispanic + PctChildren + PctElderly | ID + Year ,
...
## Comparing Rivals with Incumbents
```{r}
  ...
  df.z.dist %>% left_join(df.comp, by = c("BHCMIS ID.y" = "BHCMISID", "Year")) %>% 
      mutate(Type = "Incumbent") %>% 
    group_by(`BHCMIS ID.y`)
  
  length(unique(df.z.temp2$`BHCMIS ID`))
...
```{r}
coeffs.strat <- coeffs
coeffs.strat[2,] <- coeffs.strat[2,] / 100

strat.zip.temp <- data.frame(cbind(colnames(df.z.temp)[1:9], (coeffs.strat[-1,]))) %>% 
  #select(V1, Estimate, Pr...t.., Estimate.1, Pr...t...1) %>% 
  mutate(Estimate = round(as.numeric(Estimate), 2), `95% CI` = paste(round(as.numeric(Estimate) - 1.96 * as.numeric(`Std..Error`), 2), ", ", round(as.numeric(Estimate) + 1.96 * as.numeric(`Std..Error`), 2), sep = "")) %>% 
  mutate(Estimate.1 = round(as.numeric(Estimate.1), 2), `95% CI.1` = paste(round(as.numeric(Estimate.1) - 1.96 * as.numeric(`Std..Error.1`), 2), ", ", round(as.numeric(Estimate.1) + 1.96 * as.numeric(`Std..Error.1`), 2), sep = "")) %>% 
  filter(V1 != "ZipHHI") %>% 
   mutate(LB = Estimate - as.numeric(Std..Error) * 1.96, 
         UB = Estimate + as.numeric(Std..Error) * 1.96) %>% 
  select(V1, Estimate, LB, UB) %>% 
    mutate(Class = "Directly Competitive")%>% 
  rbind(data.frame(cbind(colnames(df.z.temp)[1:9], (coeffs.strat[-1,]))) %>% 
  #select(V1, Estimate, Pr...t.., Estimate.1, Pr...t...1) %>% 
  mutate(Estimate = round(as.numeric(Estimate), 2), `95% CI` = paste(round(as.numeric(Estimate) - 1.96 * as.numeric(`Std..Error`), 2), ", ", round(as.numeric(Estimate) + 1.96 * as.numeric(`Std..Error`), 2), sep = "")) %>% 
  mutate(Estimate.1 = round(as.numeric(Estimate.1), 2), `95% CI.1` = paste(round(as.numeric(Estimate.1) - 1.96 * as.numeric(`Std..Error.1`), 2), ", ", round(as.numeric(Estimate.1) + 1.96 * as.numeric(`Std..Error.1`), 2), sep = "")) %>% 
  filter(V1 != "ZipHHI") %>% 
   mutate(LB = Estimate.1 - as.numeric(Std..Error.1) * 1.96, 
          UB = Estimate.1 + as.numeric(Std..Error.1) * 1.96) %>% 
    select(V1, Estimate.1, LB, UB) %>% 
    rename(Estimate = Estimate.1) %>% 
    mutate(Class = "Indirectly Competitive")) %>% 
  rename(Outcome = V1) %>% 
  filter(Outcome != "Total.Hours") %>% 
  mutate(Measure = ifelse(Outcome %in% c("Sites","Mean.Hours"), "Access to Care", "Patients and Payer Mix"))



strat.zip.temp$Outcome <- as.factor(strat.zip.temp$Outcome)


p <- strat.zip.temp  %>% 
  filter(Outcome %notin% c("Intensity","CostPerPat")) %>% 
...
es.zips.none <- cbind(YearRel = c(gsub("rel_year::", "", rownames(es.direct$coeftable)), gsub("rel_year::", "", rownames(es.indirect$coeftable))), 
      rbind(cbind(Group = "Directly",es.direct$coeftable), cbind(Group = "Indirectly",es.indirect$coeftable))) %>% 
  data.frame() %>% 
  mutate(estimate = as.numeric(Estimate), 
         std.error = as.numeric(Std..Error), 
         YearRel = as.numeric(YearRel)) %>% 
 ...
 ggplot(aes(x = YearRel, y = estimate, color = Group)) + geom_point(position = position_dodge(0.9), size = 3) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.25, position = position_dodge(0.9), size = 0.8)  +
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ #theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) +
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + xlab("Year Relative to Competitive Shock") + ylab("Percentage Points") + ggtitle("Uninsured, %")



ggarrange(es.zips.hours, es.zips.sites, es.zips.mcd, es.zips.none, nrow = 2, ncol = 2, common.legend = TRUE)
...
df.z.temp %>% 
  ...
  mutate(PctMedicaid = Medicaid / MCD.denom * 100, 
         ...
         PctElderly = Age65andOver / TotalPop * 100, 
         PctEducLow = EducLow / TotalPop * 100) %>% 
    select(Status, Year, TotalPop, PctMedicaid:PctEducLow, UrbanRuralFlag) %>% 
...
df.z.temp %>% 
  ...
  mutate(PctMedicaid = Medicaid / MCD.denom * 100, 
         ...
         PctElderly = Age65andOver / TotalPop * 100, 
         PctEducLow = EducLow / TotalPop * 100) %>% 
    select(Zip, Status, Year, TotalPop, PctMedicaid:PctEducLow) %>% 
   #group_by(Zip) %>% 
  pivot_longer(cols = TotalPop:PctEducLow) %>% 
  filter(!is.na(Status)) %>% 
  unique() %>% 
  pivot_wider(id_cols = c(Zip, name, Status), names_from = Year, values_from = value) %>% 
  mutate(diff = `2022` - `2012`) %>% 
  pivot_wider(id_cols = c(Zip, Status), names_from = name, values_from = diff) %>% 
  select(Status:PctEducLow) %>% 
       tbl_summary(by = Status, 
                statistic = list(all_continuous() ~ "{mean} ({sd})"), missing = "no" ) %>% 
  add_p()
...
# Save Datasets
```{r}
# df.z.temp %>% write_csv("/Users/markow/Dropbox/Git/Competition/data_processed/finalfinal_manuscript_allocation.csv")
# df.temptemp %>% write_csv("/Users/markow/Dropbox/Git/Competition/data_processed/finalfinal_manuscript_core.csv")
```
