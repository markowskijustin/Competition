...
df.temptemp <- df.mt %>% 
  ...
  select(value, ID, MS, Year, Pct.NHA:Pct.Homeless, Qcomp, Complexity, Intensity, Pct.None:Pct.COM, CostPerPat, Altman, Staff) %>% 
  unique() 
...
data.frame(cbind(colnames(df.temptemp)[14:23], (coeffs[-1,])))
...
df.temptemptemp <- df.temptemp %>% select(Qcomp, MS, value, Year, ID, Pct.NHA:Pct.COM) %>% ungroup() %>% filter(complete.cases(.))
...
firststage <- lm(data = df.temptemptemp, I(MS/10) ~ value + factor(Year) + factor(ID) + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless )
