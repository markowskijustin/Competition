...
df.mt <- dfc %>% 
  ...
  mutate(logPatsPerStaff = ifelse(is.infinite(log(Total / Staff)), NA, (Total / Staff))) %>% 
...
df.temptemp <- df.temptemp %>% 
  ...
  mutate(Att = ifelse(Att == 2009, 0, Att))


df.temptemp <- df.temptemp #%>% filter(Att %notin% c(2011))

table(df.temptemp$Att)
...
```{r}
 ...
 event_study(data = df.temptemp,
  ...
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("FirstStage: ") + xlab("Year Relative to Competitive Shock") + ylab("Percentage Points")
   ...
   event_study(data = df.temptemp %>% mutate(ifelse(Att == 2009, 0, Att)),
  yname = "Complexity", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + UrbanRuralFlag) %>%
  filter(abs(term) <= 8) %>%
  filter(estimator == "Gardner (2021)") %>%
  #filter(estimator == "Callaway and Sant'Anna (2020)") %>%
  ggplot(aes(x = term, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9), size = 3) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.25, position = position_dodge(0.9), size = 0.8)  +
...
for (k in 1:9){
  ...
  df.z.temp2 <- df.z.temp %>% filter(Year >= 2012) %>% filter(Att %notin% c(2011))
...
es.direct <- did2s(
  data = df.z.temp %>% filter(Year >= 2012) %>% filter(Att %notin% c(2011)) %>% mutate(rel_year = ifelse(Att == 0, Inf, Year - Att)) %>% filter(Att == 0 | DirectlyTreated == 1), 
  yname = "Total.Hours", 
  treatment = "Tx",
...
es.indirect <- did2s(
  data = df.z.temp %>% filter(Year >= 2012) %>% filter(Att %notin% c(2011)) %>% mutate(rel_year = ifelse(Att == 0, Inf, Year - Att)) %>% filter(Att == 0 | DirectlyTreated == 0), 
  yname = "Total.Hours", 
  treatment = "Tx",
  first_stage = ~ 0 | ID + Year, 
    second_stage = ~ i(rel_year, ref = c(-1, Inf)) ,
...
nrow(df.z.dist.plot[df.z.dist.plot$DistToClosestSite < 10,])/nrow(df.z.dist.plot)
