...
# df.temptemp <- df.temptemp %>% 
#   left_join(uds.cw %>% filter(Year == 2011) %>% select(BHCMIS.ID:PH), by = c("BHCMIS ID" = "BHCMIS.ID")) %>% 
...
for (k in 1:28){
  ...
  model <- feols(Outcome ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + MHC + HO + PH | ID + Year | I((10000 - ClinicHHI)/1000) ~ value, cluster = "ID", data = df.temptemptemp)
...
summary(feols(df.temptemp %>% mutate(Tx = ifelse(Tx > 1, 1, 0)) %>% filter(!is.na(Tx)), I( Pct.None  ) ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless | ID + Year | I((10000 - ClinicHHI)/100) ~ Tx, cluster = "ID"))
...
```{r}
 ...
 event_study(data = df.temptemp,
  ...
  yname = "Staff", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless) %>%
  filter(abs(term) <= 8) %>%
  filter(estimator == "Gardner (2021)") %>%
  #filter(estimator == "Callaway and Sant'Anna (2020)") %>%
  ggplot(aes(x = term, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9), size = 3) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.25, position = position_dodge(0.9), size = 0.8)  +
...
for (k in 1:22){
  ...
  model <-  did2s(
      ...
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless | ID + Year , #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None | ID + Year , 
        second_stage = ~ i(Tx, ref = FALSE),
        cluster_var = "ID",
      verbose = TRUE
    )
...
core.did.estimates <- data.frame(cbind(colnames(df.temptemp)[17:38], (coeffs.did[-1,]))) %>% 
  #select(V1, Estimate, Pr...t.., Estimate.1, Pr...t...1) %>% 
  mutate(Estimate = round(as.numeric(Estimate), 2), `95% CI` = paste(round(as.numeric(Estimate) - 1.96 * as.numeric(`Std..Error`), 2), ", ", round(as.numeric(Estimate) + 1.96 * as.numeric(`Std..Error`), 2), sep = "")) %>% 
  select(V1, Estimate, `95% CI`)
...
core.did.estimates %>% 
  ...
  kable_styling(latex_options = c("striped")) %>%
  pack_rows(index = c("Overall Impacts" = 4, "Quality of Care" = 6, "Patient Complexity" = 2, "Payer Mix" = 4, "Finances" = 6)) %>% 
  add_header_above(c("DID-Estimated Effects of Competitive Shocks on Incumbent Clinics" = 3))



did2s(
      data = df.temptemp %>% mutate(Tx = ifelse(Tx > 1, 1, 0)) %>% filter(!is.na(Tx)) %>% mutate(Mix = Pct.COM * Total / 100) %>% filter(Year < 2020 ), 
      yname = "Closed", 
      treatment = "Tx",
        first_stage = ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless | ID + Year , #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None | ID + Year , 
        second_stage = ~ i(Tx, ref = FALSE),
...
df.temptemp %>% 
    ungroup() %>% 
  group_by(Tx) %>% 
  summarise(mean(Qcomp, na.rm = TRUE))
...
## TWFE First Stage
df.temptemptemp <- df.temptemp %>% select(ClinicHHI, Qcomp, MS, value, Year, ID, Pct.NHA:Pct.None, Tx) %>% ungroup() %>% filter(complete.cases(.))
...
model <- lm(data = df.temptemptemp, Complexity ~ firststage$fitted.values + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless+ factor(Year) + factor(ID))
...
reduced <- lm(data = df.temptemptemp, Complexity ~ value + factor(Year) + factor(ID) + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless )
head(summary(reduced)$coeff) %>% as.data.frame()

reduced <- lm(data = df.temptemptemp, Closed ~ value + factor(Year) + factor(ID) + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless )
head(summary(reduced)$coeff) %>% as.data.frame()
...
df.z.temp <- df.z %>% mutate(Tx = ifelse(value - 1 >= 1, 1, value - 1)) %>% filter(!is.na(Tx)) %>% mutate(DirectlyTreated = ifelse(Att.z >= 2010, 1, 0)) %>% 
  ...
  select(NumberofPatients, Pct.MCD:Pct.None, Total.Hours, Sites, Mean.Hours, Perminance, Schedule, Calendar, Tx, DirectlyTreated, ID, Year, Zip, Att, `BHCMIS ID`) 
...
for (k in 1:8){
  ...
  df.z.temp2 <- df.z.temp #%>% filter(Att == 0 | Att > 2010)
...
```{r}
 event_study(data = df.z.temp %>% filter(Att == 0 | (Att > 0 & DirectlyTreated == 0)),
  yname = "Total.Hours", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~1) %>%
  filter(abs(term) <= 8) %>%
  filter(estimator == "Gardner (2021)") %>%
  #filter(estimator == "Callaway and Sant'Anna (2020)") %>%
  ggplot(aes(x = term, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9), size = 3) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.25, position = position_dodge(0.9), size = 0.8)  +
 ...
 event_study(data = df.z.temp %>% filter(Att == 0 | (Att > 0 & DirectlyTreated == 1)),
  yname = "Total.Hours", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~1) %>%
  filter(abs(term) <= 8) %>%
  filter(estimator == "Gardner (2021)") %>%
  #filter(estimator == "Callaway and Sant'Anna (2020)") %>%
...
df.z.temp %>% 
  ...
  mutate(PctMedicaid = Medicaid / MCD.denom * 100, 
    ...
    select(Status, TotalPop, PctMedicaid:PctHispanic) %>% 
...
df.z <- left_join(df.temptemp, 
  ...
  mutate(Tx.z = ifelse(Clinics == 1 , 0, 
                ifelse(is.na(Clinics), NA, 1)), # | is.na(Clinics)
         ID = as.numeric(factor(paste(`BHCMIS ID`, Zip, sep = "-")))) %>%
  select(`BHCMIS ID`:Year, Att, ID, Zip:Tx.z, Tx) %>%
  arrange(`BHCMIS ID`, Zip, Year) %>% 
  unique() #%>%  inner_join(df.qual.exp %>% filter(Year == 2015 & Total > 12050) %>% select(`Grant Number`), by = c("Grant Number"))  #& Zips < 3.75 or 4.75
  # %>%   filter(MarketShare > median(MarketShare, na.rm = TRUE) - 0.25) 
...
Takes a fair bit to run (~ 15 minutes)
```{r}
...
zipdist <- df.z.temp %>%
  ...
  select(`BHCMIS ID`, Zip, Att, DirectlyTreated, Tx, ZipCode) %>%
  unique()
...
iv_model <- feols(df.iv, Qcomp ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + MHC + HO + PH | Year | I((10000 - ClinicHHI)/1000) ~ Z, cluster = "ID")
...
for (k in 1:28){
  ...
  #model <- feols(Outcome ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless   | I((10000 - ClinicHHI)/1000) ~ Tx + factor(Year) + factor(ID) , cluster = "ID", data = df.temptemptemp)
  #coeffs <- rbind(coeffs, model$coeftable[2,])
  model <- feols(Outcome ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless  | Year | I((10000 - ClinicHHI)/1000) ~ Z, cluster = "ID", data = df.temptemptemp)
  coeffs <- rbind(coeffs, model$coeftable[1,])
...
df.z.temp2 <- df.z.temp %>% 
  left_join(df.iv %>% ungroup() %>%  select(`BHCMIS ID`, Z, MHC, HO, PH) %>% unique(), by = c("BHCMIS ID")) %>% 
  left_join(df.temptemp %>% ungroup() %>% select(`BHCMIS ID`, Year, ClinicHHI), by = c("BHCMIS ID", "Year"))
