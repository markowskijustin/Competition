...
df.temptemp <- df.mt %>% 
  ...
  select(ClinicHHI, Att, `BHCMIS ID`, value, ID, MS, Year, Pct.NHA:Pct.Homeless, Qcomp, Total, Altman, logPatsPerStaff, 
         htn.rate, dm.rate, pap.rate, crc.rate, childbmi.rate, adultbmi.rate, Complexity, Intensity, Pct.MCD, Pct.MCR, Pct.COM, Pct.None, CostPerPat, quick_ratio, months_of_cash, Profitability, LiabPctAssets, Staff, Closed) %>% 
  unique() 
...
core.iv.estimates <- data.frame(cbind(colnames(df.temptemp)[17:38], (coeffs[-1,]))) %>% 
...
model <- feols(Complexity ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless   | I((10000 - ClinicHHI)/1000) ~ Tx  + factor(Year) + factor(ID), cluster = "ID", data = df.temptemp)
summary(model)


model <- lm(data = df.temptemp, Pct.None ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless   + Tx + factor(Year) + factor(ID) )
summary(model)
...
## TWFE First Stage
df.temptemptemp <- df.temptemp %>% select(ClinicHHI, Qcomp, MS, value, Year, ID, Pct.NHA:Pct.None) %>% ungroup() %>% filter(complete.cases(.))
...
```{r}
 event_study(data = df.z.temp %>% filter(Att == 0 | DirectlyTreated == 0),
 ...
 event_study(data = df.z.temp %>% filter(Att == 0 | DirectlyTreated == 1),
  yname = "Total.Hours", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~1) %>%
  filter(abs(term) <= 6) %>%
  filter(estimator == "Gardner (2021)") %>%
  #filter(estimator == "Callaway and Sant'Anna (2020)") %>%
