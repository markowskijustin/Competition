...
assliabs <- read.csv("/Users/jhm65/Dropbox/Git/Data/FQHC_Assets_and_Liabilities.csv")
  ...
  mutate(Qcomp = (dm.rate + htn.rate + crc.rate + pap.rate + childbmi.rate + adultbmi.rate ) / 6 * 100, 
         Qcomp.w = (dm.rate * dm.total + htn.rate * htn.total + crc.rate * crc.denom + pap.rate * pap.denom + childbmi.rate * childbmi.denom + adultbmi.rate * adultbmi.denom ) / 
           ( dm.total +  htn.total +  crc.denom +  pap.denom +  childbmi.denom +  adultbmi.denom ) * 100, 
               ...
               Profitability = ngla_net_gain_loss / Assets, 
               Efficiency = ngla_revenue / Assets,
...
dfc <- dfc %>% 
  mutate(Liquidity = ifelse(is.infinite(Liquidity), NA, Liquidity), 
         ...
         Altman = 6.56 * Liquidity + 3.26 * Profitability + 6.72 * Efficiency + 1.05 * NetWorth) %>% 
...
## Write the file
```{r}
# write_csv(dfc, file = "/Users/jhm65/Dropbox/Git/Competition/data_processed/analyticalfile.csv")
# write.csv(uds.sites.full, file = "/Users/jhm65/Dropbox/Git/Competition/data_processed/sites_geocoded.csv")
```
