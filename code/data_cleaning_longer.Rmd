...
select(BHCMISID:FIPS, ID, Total:Pct.COM, imm.rate:MinYear)
...
sites.in.zips <- uds.sites.full %>% 
  filter(Year >= included.years[1]) %>% 
  group_by(`BHCMIS ID`, Zip, Year) %>% 
  summarise(Counts = n()) %>% 
  pivot_wider(id_cols = c(Zip, `BHCMIS ID`), names_from = Year, values_from = Counts) %>% 
  pivot_longer(cols = `2010`:`2021`) %>% 
  mutate(value = ifelse(is.na(value), 0, value)) %>% 
  arrange(Zip, name)

clinics.in.zips <- uds.sites.full %>% 
  filter(Year >= included.years[1]) %>%
  select(`BHCMIS ID`, Zip, Year) %>% 
  unique() %>% 
  group_by(Zip, Year) %>% 
  summarise(Counts = n()) %>% 
  pivot_wider(id_cols = c(Zip), names_from = Year, values_from = Counts) %>% 
  pivot_longer(cols = `2011`:`2017`) %>% 
  mutate(value = ifelse(is.na(value), 0, value)) %>% 
  arrange(Zip, name)


sites.in.zips %>% 
  left_join(clinics.in.zips, by = c("Zip","name"))%>% 
...
## Write the file
```{r}
#write_csv(df.comp, file = "/Users/jhm65/Dropbox/Projects/Competition/data_processed/analyticalfile.csv")
