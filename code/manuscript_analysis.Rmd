...
cbind(colnames(df.spill3)[c(30, 10:19)], TEs.strat.far %>% mutate(Estimate = round(Estimate, 2), `95% CI` = paste(round(Estimate - 1.96 * `Std..Error`, 2), ", ", round(Estimate + 1.96 * `Std..Error`, 2), sep = "")) %>% select(Estimate, `95% CI`, `Pr...t..`), TEs.strat.close%>% mutate(Estimate = round(Estimate, 2), `95% CI` = paste(round(Estimate - 1.96 * `Std..Error`, 2), ", ", round(Estimate + 1.96 * `Std..Error`, 2), sep = "")) %>% select(Estimate, `95% CI`, `Pr...t..`))[c(7:11, 2, 1, 3:6),]


did2s(
      data = df.spill3  %>% filter((Dist > quantile(Dist, 0.85) & Att > 0) | Att == 0 ) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% 
        mutate(Outcome = ifelse(Sites == 0, 0, Schedule / Sites)), 
        #mutate(Tx = ifelse(Att - 1 == Year, 1, Tx)) %>% 
       # mutate(Total.Hours = ifelse(Total.Hours > quantile(Total.Hours, 0.975, na.rm = TRUE), quantile(Total.Hours, 0.975, na.rm = TRUE), 
        #                            ifelse(Total.Hours < quantile(Total.Hours, 0.025, na.rm = TRUE), quantile(Total.Hours, 0.025, na.rm = TRUE), Total.Hours))) , 
      yname = "Outcome", 
...
dist.close <- mean(df.spill3[df.spill3$Dist < quantile(df.spill3$Dist, probs = c(0.15)),]$Dist)
dist.far <- mean(df.spill3[df.spill3$Dist > quantile(df.spill3$Dist, probs = c(0.85)),]$Dist)

df.spill3 %>% filter(Att > 0 & Dist < quantile(df.spill3$Dist, probs = c(0.15)) & Year == Att - 1) %>% summarise(sum(Total.Hours), n())
df.spill3 %>% filter(Att > 0 & Dist > quantile(df.spill3$Dist, probs = c(0.85)) & Year == Att - 1) %>% summarise(sum(Total.Hours), n())


df.spill3 %>% 
  filter(Year == Att - 1) %>% 
  mutate(EstimatedChange =  TEs.strat.close[2,1] + (TEs.strat.far[2,1] - TEs.strat.close[2,1]) / (dist.far - dist.close)*Dist) %>% 
  summarise(sum(abs(EstimatedChange)) / sum(Total.Hours))
...
df.spill3 %>% 
  ...
  filter(!is.na(DistFilter) & Year < 2013) %>% 
...
ggsave(filename = "AppendixFigures10.png", path = "/Users/markow/Dropbox/Writing/CHC Competitive Shocks/Figures", plot = p.rival.dist, width = 10, height = 6)
```
...
df.p %>% 
 ...
 df.comp %>% 
  ...
  add_p()
    ...
    df.z.dist %>% 
      mutate(Year = Year - 1) %>% 
      left_join(df.comp, by = c("BHCMIS ID.y" = "BHCMISID", "Year")) %>% 
      ...
      select(Type, Quality, Total, Altman, logPatsPerStaff, Pct.NHA:Pct.COM) %>% 
          tbl_summary(by = Type, 
                statistic = list(all_continuous() ~ "{mean} ({sd})"), 
