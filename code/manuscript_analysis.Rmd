...
df.core <- dfc %>% 
  ...
  mutate(MCD = Total * Pct.MCD / 100, 
         ...
         dm.rate = dm.rate * 100, 
         logPatsPerStaff = log(Total / Staff), 
         logPatsPerStaff = ifelse(log(Total / Staff) > quantile(logPatsPerStaff, 0.975, na.rm = T), quantile(logPatsPerStaff, 0.975, na.rm = T), 
                                  ifelse(log(Total / Staff) < quantile(logPatsPerStaff, 0.025, na.rm = T), quantile(logPatsPerStaff, 0.025, na.rm = T), log(Total / Staff)))) %>% 
  #mutate(Qcomp = (crc.rate + pap.rate + adultbmi.rate + childbmi.rate) / 4) %>% 
  select(`BHCMIS ID`, value, Tx, Year, Att, Att.random, year.rel, ID, STATE, 
         Qcomp, Altman, logPatsPerStaff, #MCD:COM, 
...
event_study(data = df.core,
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) +
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5)  + ggtitle("Financial Resilience")+ xlab("Year Relative to Competitive Shock") + ylab("Risk Units"),
event_study(data = df.core,
  yname = "logPatsPerStaff", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless) %>%
  filter(abs(term) <= 5) %>%
  filter(estimator == "Gardner (2021)") %>%
  ggplot(aes(x = term, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9), color = 'goldenrod', size = 3) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.25, position = position_dodge(0.9), color = 'goldenrod', size = 0.8)  +
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) +
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5)  + ggtitle("Operational Efficiency") + xlab("Year Relative to Competitive Shock") + ylab("log Ratio Patients-to-Staff") , common.legend = TRUE, ncol = 1 )
p0
...
summary(lm(data = df.core %>% filter(!is.na(Tx) & year.rel < 0) %>% mutate(Treated = ifelse(Att > 0, 1, 0)) %>% filter(!is.infinite(logPatsPerStaff)), logPatsPerStaff ~ Treated*year.rel))$coeff[4,]

ggsave(filename = "Figure2.png", path = "/Users/markow/Dropbox/Writing/CHC Competitive Shocks/Figures", plot = p0, width = 9, height =16)


label1 <- 'beta[pooled] == "2.24**"'
label2 <- 'beta[pooled] == "-0.21**"'
label3 <- 'beta[pooled] == "0.05**"'
...
#   theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
#   geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Quality Composite") + xlab("Year Relative to Competitive Shock") + ylab("Percentage Points")+ 
#      annotate("text", x=2, y=-2, label= label1, parse = TRUE, size = unit(10, "pt"))+theme(axis.text=element_text(size=12),
...
#   theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
#   geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5)  + ggtitle("Financial Resilience")+ xlab("Year Relative to Competitive Shock") + ylab("Risk Units")+ 
#      annotate("text", x=-2, y=-0.3, label= label2, parse = TRUE, size = unit(10, "pt"))+theme(axis.text=element_text(size=12),
#      axis.title=element_text(size=14,face="bold")), 
# event_study(data = df.core, 
#   yname = "logPatsPerStaff", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless) %>% 
#   filter(abs(term) <= 5) %>% 
...
#   #facet_wrap(~V1, scales = "free_y", ncol = 3) +
#   theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
#   geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5)  + ggtitle("Operational Efficiency") + xlab("Year Relative to Competitive Shock") + ylab("log Ratio Patients-to-Staff") + 
#      annotate("text", x=2, y=-0.05, label= label3, parse = TRUE, size = unit(10, "pt"))+theme(axis.text=element_text(size=12),
#      axis.title=element_text(size=14,face="bold")) , common.legend = TRUE, ncol = 3 ) 
# p0
...
for (j in 10:12){
  ...
  colnames(df.temp)[10] <- "Outcome"
  outcomes <- did2s(
      data = df.temp %>% mutate(Tx = ifelse(Att - 1 == Year, 1, Tx))  , 
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless | ID + Year , #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None | ID + Year , 
        second_stage = ~ i(Tx, ref = FALSE),
...
mean.nas<- function(x){ mean(x, na.rm = TRUE)}
mean.inf<- function(x){ mean(ifelse(is.infinite(x), NA, x), na.rm = TRUE)}
...
did2s(
      data = dfc %>% mutate(outcome = log(ngla_revenue / Staff)) %>% #filter(outcome < 15 & !is.infinite(outcome)) %>% 
        mutate(Tx = ifelse(Att - 1 == Year, 1, Tx)), 
      yname = "Staff", 
      treatment = "Tx",
        first_stage = ~ Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless | ID + Year , #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None | ID + Year , 
        second_stage = ~ i(Tx, ref = FALSE),
...
#ggsave(filename = "Picture14.png", path = "/Users/jhm65/Dropbox/Writing/JMP_figures", plot = p14, width = 17, height = 9)
 ...
 event_study(data = dfc %>% mutate(outcome = ngla_revenue / Total),
  yname = "outcome", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless) %>%
  #mutate(estimate = exp(estimate), std.error = exp(std.error)) %>% 
  filter(abs(term) <= 5) %>%
  filter(estimator == "Gardner (2021)") %>%
...
p2 <- ggarrange(
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6) + theme(legend.position = "none", plot.title = element_text(size=18, face='bold'))+ 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Complexity") + xlab("Year Relative to Competitive Shock")  + ylab("Prevalence")+theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")), 
event_study(data = df.select.stint, 
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6) + theme(legend.position = "none", plot.title = element_text(size=18, face='bold'))+ 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5)  + ggtitle("Intensity") + xlab("Year Relative to Competitive Shock") + ylab("Visits")+theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold")), common.legend = TRUE, legend = F) 
p2
...
ggsave(filename = "Figure3.png", path = "/Users/markow/Dropbox/Writing/CHC Competitive Shocks/Figures", plot = p2, width = 17, height = 9)
...
## Rival selecting where to put new site, takes 15+ minutes to work
...
df.spill3 <- df.spill2 %>% #filter(NumberofPatients > 50 ) %>% 
  ...
  # inner_join(df.spill2 %>% 
  # group_by(Zip, `BHCMIS ID`) %>% 
  # summarise(maxmax = min(NumberofPatients)) %>% 
  #   filter(maxmax > 100), by = c("Zip","BHCMIS ID"))  %>% 
  
  inner_join(df.spill2 %>% 
  group_by(Zip, `BHCMIS ID`) %>% 
  summarise(counts = n()) %>% 
    filter(counts == 10), by = c("Zip","BHCMIS ID"))  %>%
  filter(Att == 0 | Att > 2013) %>% 
...
## Access shifts by quantiles, event studies
```{r}

    
    
zp1 <- event_study(data = df.spill3 %>% filter(Dist > quantile(Dist, 0.87)), 
...
zp3 <- event_study(data = df.spill3 %>% filter((Dist > quantile(Dist, 0.87) & Att > 0) | Att == 0) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites)), 
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6) + theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Hours per Week in Farthest 13% of Zips")+ xlab("Year Relative to Competitive Shock") + ylab("Hours per Week")+theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))



zp4 <- event_study(data = df.spill3 %>% filter((Dist == 0 & Att > 0) | Att == 0 ) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites)), 
  yname = "Total.Hours", idname = "Zip", tname = "Year", gname = "Att", estimator = "all") %>% 
  filter(abs(term) < 6) %>% 
  filter(estimator %in% c("Gardner (2021)")) %>% 
  ggplot(aes(x = term, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9), color = 'goldenrod', size = 3) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.25, position = position_dodge(0.9), color = 'goldenrod', size = 0.8)  +
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6) + theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Hours per Week in Competitive Zip")+ xlab("Year Relative to Competitive Shock") + ylab("Hours per Week")+theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))
...
did2s(
      data = df.spill3  %>% filter((Dist > quantile(Dist, 0.85) & Att > 0) | Att == 0) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% 
        mutate(Total.Hours = ifelse(Total.Hours > quantile(Total.Hours, 0.975, na.rm = TRUE), quantile(Total.Hours, 0.975, na.rm = TRUE), 
                                    ifelse(Total.Hours < quantile(Total.Hours, 0.025, na.rm = TRUE), quantile(Total.Hours, 0.025, na.rm = TRUE), Total.Hours))), 
      yname = "Total.Hours", 
      treatment = "Tx",
        first_stage = ~ 1 | Zip + Year , 
        second_stage = ~ i(Tx, ref = FALSE)  ,
...
did2s(
      data = df.spill3  %>% filter((Dist < quantile(Dist, 0.15) & Att > 0) | Att == 0) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% 
        mutate(Total.Hours = ifelse(Total.Hours > quantile(Total.Hours, 0.975, na.rm = TRUE), quantile(Total.Hours, 0.975, na.rm = TRUE), 
                                    ifelse(Total.Hours < quantile(Total.Hours, 0.025, na.rm = TRUE), quantile(Total.Hours, 0.025, na.rm = TRUE), Total.Hours))), 
      yname = "Total.Hours", 
      treatment = "Tx",
...
## Acces shifts by quantiles
```{r}
qnats1 <- seq(0, 0.875, by = 0.125)
qnats2 <- seq(0.125, 1, by = 0.125)


qnats1 <- seq(0, 4/5, by = 1/5)
qnats2 <- seq(1/5, 1, by = 1/5)
   ...
   model.dist.quants <-  did2s(
      data = df.spill3 %>% filter((Dist == 0 & Att > 0) | Att == 0) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% mutate(Tx = ifelse(Att - 1 == Year & Att > 0, 1, Tx))%>% 
        mutate(Total.Hours = ifelse(Total.Hours > quantile(Total.Hours, 0.975, na.rm = TRUE), quantile(Total.Hours, 0.975, na.rm = TRUE), 
                                    ifelse(Total.Hours < quantile(Total.Hours, 0.025, na.rm = TRUE), quantile(Total.Hours, 0.025, na.rm = TRUE), Total.Hours))),  
      yname = "NumberofPatients", 
      treatment = "Tx",
  ...
  for (i in 1:(length(qnats1))){
      model.dist.quants <-  did2s(
      data = df.spill3 %>%  filter((Dist > quantile(Dist, qnats1[i]) & Dist <= quantile(Dist, qnats2[i]) & Att > 0 ) | Att == 0) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% mutate(Tx = ifelse(Att - 1 == Year & Att > 0 , 1, Tx))  %>% 
        mutate(Total.Hours = ifelse(Total.Hours > quantile(Total.Hours, 0.975, na.rm = TRUE), quantile(Total.Hours, 0.975, na.rm = TRUE), 
                                    ifelse(Total.Hours < quantile(Total.Hours, 0.025, na.rm = TRUE), quantile(Total.Hours, 0.025, na.rm = TRUE), Total.Hours))), 
      yname = "NumberofPatients", 
      treatment = "Tx",
...
zp.pats <- tes[-1,] %>% 
  cbind(Quantile = 1:(length(qnats1))) %>% 
  ggplot(aes(x = Quantile, y = Estimate)) + geom_point(position = position_dodge(0.9), color = 'goldenrod', size = 3) + geom_errorbar(aes(ymin=Estimate-1.96*Std..Error, ymax=Estimate+1.96*Std..Error), width=.25, position = position_dodge(0.9), color = 'goldenrod', size = 0.8)  +
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Effects on Total Patients, by Quantile from Competitive Shock")+ xlab("Quantile of Zip Codes by Distance from Competitive Shock, adjusting for 1 year anticipation") + ylab("Effect Estimate")

zp.pats

quantile(df.spill3[df.spill3$Att > 0,]$Dist, 1/3)
quantile(df.spill3[df.spill3$Att > 0,]$Dist, 2/3)

#ggsave(filename = "Picture13pats.png", path = "/Users/jhm65/Dropbox/Writing/JMP_figures", plot = zp.pats, width = 16, height = 9)

summary(df.spill3$l.popdensity)
```



## Stratified analysis instead for Access
```{r}

#### STRATIFEID ANALYSIS INSTEAD

quantile(df.spill3$Dist, 0.15, na.rm = TRUE)
...
TEs.strat.far <- data.frame(model$coeftable)[1,]

for (i in 11:19){
  df.temp <- df.spill3 %>% filter((Dist > quantile(Dist, 0.85) & Att > 0) | Att == 0 ) #%>% 
        #mutate(Tx = ifelse(Att - 1 == Year, 1, Tx))
  df.temp <- df.temp[,c(1:5, i, 16:ncol(df.spill3))] 
  names(df.temp)[1:6] <- c("Tx", "D", "Dist", "Year", "Zip", 'Outcome')
  model <- did2s(
      data = df.temp , 
      yname = "Outcome", 
      treatment = "Tx",
  ...
  TEs.strat.far <- rbind(TEs.strat.far, data.frame(model$coeftable)[1,])
}
...
TEs.strat.close <- data.frame(model$coeftable)[1,]

for (i in 11:19){
  df.temp <- df.spill3 %>% filter((Dist < quantile(Dist, 0.15) & Att > 0) | Att == 0 ) #%>%  mutate(Tx = ifelse(Att - 1 == Year, 1, Tx))
  df.temp <- df.temp[,c(1:5, i, 16:ncol(df.spill3))] 
  names(df.temp)[1:6] <- c("Tx", "D", "Dist", "Year", "Zip", 'Outcome')
  model <- did2s(
      data = df.temp , 
  ...
  TEs.strat.close <- rbind(TEs.strat.close, data.frame(model$coeftable)[1,])
}



cbind(colnames(df.spill3)[10:19], TEs.strat.far %>% mutate(Estimate = round(Estimate, 2), `95% CI` = paste(round(Estimate - 1.96 * `Std..Error`, 2), ", ", round(Estimate + 1.96 * `Std..Error`, 2), sep = "")) %>% select(Estimate, `95% CI`, `Pr...t..`), TEs.strat.close%>% mutate(Estimate = round(Estimate, 2), `95% CI` = paste(round(Estimate - 1.96 * `Std..Error`, 2), ", ", round(Estimate + 1.96 * `Std..Error`, 2), sep = "")) %>% select(Estimate, `95% CI`, `Pr...t..`))[c(6:10, 1:2),]
```



## Back of the envelop calculation, how much resources are reallocated?
```{r}



df.spill3  %>% filter(Dist > quantile(Dist, 0.8) ) %>% 
  filter(Att > 0) %>% 
  group_by(Tx) %>% 
  summarise(mean(Total.Hours))


df.spill3 %>% filter(Dist < quantile(Dist, 0.2)) %>% 
  filter(Att > 0) %>%
  group_by(Tx) %>% 
  summarise(mean(Total.Hours))


summary(lm(data = df.spill3 %>% filter(Att > 0 & Dist < 200), Total.Hours ~ Dist))
  
summary(df.spill3[df.spill3$Att > 0,]$Dist)


quantile(df.spill3[df.spill3$Att > 0,]$Dist, 0.17)

### REALLY IMPORTANT BACK OF THE ENVELOP CALCULATION

sum(df.spill3 %>% filter(Att > 0 & Dist < 200 & Dist > 0 & Year == Att - 1) %>% select(Total.Hours))


# Funcationl form of ATT x distance for total hours.... 1.64 - 0.041 * DIST... which integrated from 40km to 200km is about 524 lost hours
# Divide that by the proceeding line leads to ~ 5% reallocation of resources
## UPDATED FORM : 3.37 - 0.054x from 62km through 200 is 7%

#UPDATED AGAIN : 2.167812	* 	-0.427669x where X is in 10km increments

integrand <- function(x) {2.167812 + -0.0427669*x}
cutpoint <- 2.167812 / 0.0427669

areaUnder <- integrate(f = integrand, lower = cutpoint, upper = 200)
areaUnder$value / sum(df.spill3 %>% filter(Att > 0 & Dist < 200 & Dist > cutpoint & Year == Att - 1) %>% select(Total.Hours))

areaUnder <- integrate(f = integrand, lower = 0, upper = cutpoint)
areaUnder$value / sum(df.spill3 %>% filter(Att > 0 & Dist < cutpoint & Dist > 0 & Year == Att - 1) %>% select(Total.Hours))

df.spill3 %>% 
  filter(Year == Att - 1) %>% 
  mutate(EstimatedChange = 2.167812 + -0.0427669*Dist) %>% 
  summarise(sum(abs(EstimatedChange)) / sum(Total.Hours))



did2s(
      data = df.spill3 %>% #filter((Dist > quantile(Dist, 0.4) & Dist < quantile(Dist, 0.6) & Att > 0) | Att == 0 ) %>% 
        mutate(Total.Hours = ifelse(Total.Hours > quantile(Total.Hours, 0.975, na.rm = TRUE), quantile(Total.Hours, 0.975, na.rm = TRUE), 
                                    ifelse(Total.Hours < quantile(Total.Hours, 0.025, na.rm = TRUE), quantile(Total.Hours, 0.025, na.rm = TRUE), Total.Hours))), 
      yname = "Total.Hours", 
      treatment = "Tx",
...
mean(df.spill3[df.spill3$Dist < quantile(df.spill3$Dist, probs = c(0.15)),]$Dist)
mean(df.spill3[df.spill3$Dist > quantile(df.spill3$Dist, probs = c(0.85)),]$Dist)

df.spill3 %>% filter(Att > 0 & Dist < 0.33476343 & Year == Att - 1) %>% summarise(sum(Total.Hours), n())
df.spill3 %>% filter(Att > 0 & Dist > 157.1606 & Year == Att - 1) %>% summarise(sum(Total.Hours), n())

df.spill3 %>% 
  filter(Year == Att - 1) %>% 
  mutate(EstimatedChange = 6 + -0.039*Dist) %>% 
  summarise(sum(abs(EstimatedChange)) / sum(Total.Hours))
...
Checking robustness of primary findings, allowing each FQHC to make shifting decisions within their own boarders... what happens? Simiarl story

```{r}
did2s(
      data = df.spill3 %>% 
        group_by(`BHCMIS ID`) %>% 
        arrange(`BHCMIS ID`, Dist) %>% 
        mutate(QuantClose = quantile(Dist, 0.333), 
               QuantFar = quantile(Dist, 0.666), 
               QuantMid = quantile(Dist, 0.5), 
               Denom = length(unique(Dist)), 
               Num = rank(Dist), 
               Quant = as.numeric((Dist - min(Dist)) / max(Dist)), 
               PopDensity = max(l.popdensity, na.rm = TRUE)) %>% filter(!is.na(Quant)) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% 
        #mutate(Tx = ifelse(Att - 1 == Year, 1, Tx)) %>% 
        mutate(Total.Hours = ifelse(Total.Hours > quantile(Total.Hours, 0.975, na.rm = TRUE), quantile(Total.Hours, 0.975, na.rm = TRUE), 
                                    ifelse(Total.Hours < quantile(Total.Hours, 0.025, na.rm = TRUE), quantile(Total.Hours, 0.025, na.rm = TRUE), Total.Hours))) %>% 
        mutate(MedicaidRate = Medicaid / NumberofPatients * 100, 
               UninsuredRate = Uninsured / NumberofPatients * 100), 
      yname = "Total.Hours", 
      treatment = "Tx",
        first_stage = ~ Sites| Zip + Year , 
        second_stage = ~ i(Tx, ref = FALSE) + I(Tx * as.numeric(Quant) * 10),
        cluster_var = "Zip",
      verbose = FALSE
    ) 
...
df.spill3 %>% 
  group_by(`BHCMIS ID`) %>% 
  mutate(maxPopDens = max(l.popdensity, na.rm = TRUE)) %>% 
    unique() %>% 
    ungroup() %>% 
  select(maxPopDens)%>% unlist() %>% 
  as.numeric() %>% 
  summary()
...
df.spill3 %>% 
   mutate(DistFilter = ifelse(Dist < quantile(Dist, 0.15), "Near", 
                              ifelse(Dist > quantile(Dist, 0.85), "Far", NA))) %>% 
  filter(!is.na(DistFilter) & Year == 2012) %>% 
  select(DistFilter, Tx:NumberofPatients, ALAND10:l.popdensity) %>% 
  inner_join(census, by = c("Zip" = "ZipCode", "Year" = "YEAR")) %>% 
  select(DistFilter, TotalPop:Uninsured) %>% 
...
## Distribution of FQHC sites nationwide
...
p.grant <- census %>% 
  ...
  summarise(Pct.Poverty = IncomeBelowPoverty / PovertyStatusDet * 100, 
            Pct.MCD = Medicaid / MCD.denom * 100, 
            Pct.Unins = Uninsured / Unins.denom * 100) %>% 
  left_join(uds.sites.full %>% 
  filter(Year == 2021) %>% 
  group_by(Zip, Year) %>% 
  summarise(Counts = n()), by = c("ZipCode" = "Zip", "YEAR" = "Year")) %>% 
  mutate(Counts = ifelse(is.na(Counts), 0, Counts), 
         Counts_Bin = ifelse(Counts == 0, 0, 100), 
         `Poverty, %` = round(Pct.Poverty, digits = -1),
         `Medicaid, %` = round(Pct.MCD, digits = -1), 
         `Uninsured, %` = round(Pct.Unins, digits = -1)) %>% 
  pivot_longer( `Poverty, %`:`Uninsured, %`) %>% 
  ungroup() %>% group_by(name, value) %>% 
  summarise(`# Zip Codes` = n(), PropWithCHC = mean(Counts_Bin)) %>% 
  filter(!is.na(value)) %>% 
  rename(Measure = name) %>% 
  ggplot(aes(x = value, y = PropWithCHC, group_by(Measure))) + geom_point(aes(size =`# Zip Codes`, color = Measure)) + geom_line(aes(group = Measure, color = Measure))+
  theme_tufte_revised() + xlab("Percent, %") + ylab("Zips with a CHC, %") + ylim(0, 37) + #facet_wrap(~Measure) +
  theme(axis.text=element_text(size=10), axis.title=element_text(size=10,face="bold"), strip.text = element_text(size=12,face="bold"))
p.grant
...
bhoch <- data.frame(rbind(did.core, did.select.stint, did.select.payer, did.util)) %>% 
  rbind(TEs) %>% rbind(TEs_Dist) %>% 
  arrange(`Pr...t..`) %>% 
  cbind(c(1:37) / 37 * 0.1) %>% 
  mutate(Row = row_number()) %>% 
  ggplot(aes(x = Row, y = `Pr...t..`)) + geom_point() + 
  ylab("p-values") + xlab("Significance Rank") + geom_abline(slope = 1/37 * 0.1, intercept = 0, color="red", linetype="dashed", alpha=.5) + theme_minimal()
