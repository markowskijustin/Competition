...
qnats1 <- seq(0, 5/6, by = 1/6)
qnats2 <- seq(1/6, 1, by = 1/6)
...
model <- did2s(
      data = df.spill3  %>% filter((Dist > quantile(Dist, 0.87) & Att > 0) | Att == 0 ) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% 
        mutate(Tx = ifelse(Att - 1 == Year, 1, Tx))  , 
      yname = "Total.Hours", 
      treatment = "Tx",
        first_stage = ~ 1 | Zip + Year , 
...
for (i in 11:19){
  df.temp <- df.spill3 %>% filter((Dist > quantile(Dist, 0.87) & Att > 0) | Att == 0 )%>% 
        mutate(Tx = ifelse(Att - 1 == Year, 1, Tx))
