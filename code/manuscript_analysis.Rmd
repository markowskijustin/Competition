...
plot0 <- p %>% 
  ...
  geom_dl(label=c(rep("0 Competitors", length(unique(p$Year))), rep("1 Competitor", length(unique(p$Year))), rep("2 Competitors", length(unique(p$Year))), rep("3+ Competitors", length(unique(p$Year)))), 
          method = list("last.points", hjust = 1, vjust = c(3, 3.5, 4, -2), cex = 1.5), inherit.aes=T ) + 
...
p0 <- ggarrange(
  ...
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Total Patients") + xlab("Year Relative to Competitive Shock") + ylab("Percentage Points"), 
event_study(data = df.core, 
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5)  + ggtitle("Quality Composite")+ xlab("Year Relative to Competitive Shock") + ylab("Risk Units"), 
event_study(data = df.core, 
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5)  + ggtitle("Financial Resilience") + xlab("Year Relative to Competitive Shock") + ylab("log Ratio Patients-to-Staff"), common.legend = TRUE, ncol = 1 ) 
p0
...
#ggsave(filename = "Picture3a.png", path = "/Users/jhm65/Dropbox/Writing/JMP_figures", plot = p0, width = 9, height =16)
...
event_study(data = df.core, 
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) +
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6)+ theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5)  + ggtitle("Financial Resilience") + xlab("Year Relative to Competitive Shock") + ylab("log Ratio Patients-to-Staff"), common.legend = TRUE, ncol = 3 ) 
p0
...
#   annotate("text", x=2, y=-2.2, label= label1, parse = TRUE, size = unit(10, "pt"))+theme(axis.text=element_text(size=12),
#         axis.title=element_text(size=14,face="bold")), 
# event_study(data = df.core, 
#   yname = "Altman", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless) %>% 
#   filter(abs(term) <= 5) %>% 
...
#   #facet_wrap(~V1, scales = "free_y", ncol = 3) +
#   theme_tufte_revised() + geom_vline(xintercept=-0.5, color="blue", linetype="dashed", alpha=.6) + theme(legend.position = "none", plot.title = element_text(size=18, face='bold')) + 
#   geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5)  + ggtitle("Altman's Z Score") + xlab("Year Relative to Competitive Shock") + ylab("Risk Units") +
#   annotate("text", x=2, y=0.4, label= label2, parse = TRUE, size = unit(10, "pt"))+theme(axis.text=element_text(size=12),
#         axis.title=element_text(size=14,face="bold")), common.legend = TRUE, ncol = 2, legend = F) 
# p0.simple
...
model <- did2s(
      data = df.spill3  %>% filter((Dist > quantile(Dist, 0.85) & Att > 0) | Att == 0 ) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% 
        mutate(Tx = ifelse(Att - 1 == Year, 1, Tx)) %>% mutate(Total.Hours = ifelse(Total.Hours > quantile(Total.Hours, 0.975, na.rm = TRUE), quantile(Total.Hours, 0.975, na.rm = TRUE), 
                                    ifelse(Total.Hours < quantile(Total.Hours, 0.025, na.rm = TRUE), quantile(Total.Hours, 0.025, na.rm = TRUE), Total.Hours))) , 
      yname = "Total.Hours", 
      treatment = "Tx",
        first_stage = ~ 1 | Zip + Year , 
...
model <- did2s(
      data = df.spill3  %>% filter((Dist == 0 & Att > 0) | Att == 0 ) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% 
        mutate(Tx = ifelse(Att - 1 == Year, 1, Tx)) %>% mutate(Total.Hours = ifelse(Total.Hours > quantile(Total.Hours, 0.975, na.rm = TRUE), quantile(Total.Hours, 0.975, na.rm = TRUE), 
                                    ifelse(Total.Hours < quantile(Total.Hours, 0.025, na.rm = TRUE), quantile(Total.Hours, 0.025, na.rm = TRUE), Total.Hours))) , 
      yname = "Total.Hours", 
      treatment = "Tx",
...
for (i in 11:19){
  df.temp <- df.spill3 %>% filter((Dist == 0 & Att > 0) | Att == 0 ) %>% 
        mutate(Tx = ifelse(Att - 1 == Year, 1, Tx))
  df.temp <- df.temp[,c(1:5, i, 16:ncol(df.spill3))] 
  names(df.temp)[1:6] <- c("Tx", "D", "Dist", "Year", "Zip", 'Outcome')
  model <- did2s(
...
df.spill3  %>% filter(Dist > quantile(Dist, 0.9) ) %>% 
  filter(Att > 0) %>% 
  group_by(Tx) %>% 
...
sum(df.spill3 %>% filter(Att > 0 & Dist < 200 & Dist > 62 & Year == Att - 1) %>% select(Total.Hours))
