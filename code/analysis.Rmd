...
```{r}
did2s(
      data = df.spill2 %>% inner_join(df.qual.exp %>% filter(Year == 2016) %>% #mutate(ZipFilter = ifelse(`Surplus as a pct of before dep expenses` < 2, 1, 2)) %>% 
               select(`Grant Number`, Pct.NHA:Pct.Children, MCD, COM, MCR), by = c("Grant Number")), 
      yname = "Total.z", 
      treatment = "Tx",
        first_stage = ~ 0 | Zip + Year + D + Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + Pct.Children + MCD + MCR + COM, 
