...
plot0 <- p %>% 
  ...
  geom_dl(label=c(rep("0 Competitors", 25), rep("1 Competitor", 25), rep("2 Competitors", 25), rep("3+ Competitors", 25)), 
          method = list("last.points", hjust = 1, vjust = c(3, 3.5, 4, -2), cex = 1), inherit.aes=T ) + 
  guides(colour = "none") 
...
plot1 <- uds.sites.full[!is.na(uds.sites.full$Zip),] %>% 
  ...
  geom_dl(label=c(rep("1 Site", 25), rep("2+ Sites", 25)), method = list("last.points", hjust = 1, vjust = -1, cex = 1), inherit.aes=T ) + 
...
zp.pats <- tes[-1,] %>% 
  ...
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Effects on Total Patients, by Quantile from Competitive Shock")+ xlab("Quantile of Zip Codes by Distance from Competitive Shock, adjusting for 1 year anticipation") + ylab("Effect Size")
