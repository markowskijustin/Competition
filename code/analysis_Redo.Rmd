...
uds.sites.full <- read_csv("/Users/jhm65/Dropbox/Git/Competition/data_processed/sites_geocoded.csv") %>%
  mutate(Access = ifelse(`BHCMIS ID` == "080170" & as.numeric(`Total Weekly Hours Of Operation`) > 40000, 40, Access))
...
df.select.stint <- dfc %>% 
  ...
  mutate(Complexity = (HIV.prev + HeartDisease.prev + Depression.prev + Diabetes.prev + Hypertension.prev) / 5, 
         ...
         PatientsPerStaff = Staff, #Total / Staff, 
...
df.spill3 <- df.spill2 %>% filter(NumberofPatients > 50) %>% 
  ...
  mutate(Poverty = IncomeBelowPoverty / PovertyStatusDet) #%>% filter(Zip != '04401')
# ,
...
zp1 <- event_study(data = df.spill3 %>% filter(Dist > quantile(Dist, 0.8)), 
  yname = "Sites", idname = "Zip", tname = "Year", gname = "Att", estimator = "all") %>% 
  filter(abs(term) < 7) %>% 
  filter(estimator %in% c("Gardner (2021)")) %>% 
  ggplot(aes(x = term, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9)) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.2, position = position_dodge(0.9))  + 
  #facet_wrap(~V1, scales = "free_y", ncol = 3) + 
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="red", linetype="dashed", alpha=.5) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Sites in Farthest 20% of Zips")
...
zp2 <- event_study(data = df.spill3 %>% filter(Dist == 0), 
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) + 
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="red", linetype="dashed", alpha=.5) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Sites in Competitive Zip")


zp3 <- event_study(data = df.spill3 %>% filter(Dist > quantile(Dist, 0.8)) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites)), 
  yname = "Hours", idname = "Zip", tname = "Year", gname = "Att", estimator = "all") %>% 
  filter(abs(term) < 7) %>% 
  filter(estimator %in% c("Gardner (2021)")) %>% 
  ggplot(aes(x = term, y = estimate, color = estimator)) + geom_point(position = position_dodge(0.9)) + geom_errorbar(aes(ymin=estimate-1.96*std.error, ymax=estimate+1.96*std.error), width=.2, position = position_dodge(0.9))  + 
  #facet_wrap(~V1, scales = "free_y", ncol = 3) + 
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="red", linetype="dashed", alpha=.5) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Total Hours in Farthest 20% of Zips")
...
zp4 <- event_study(data = df.spill3 %>% filter(Dist == 0) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites)), 
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) + 
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="red", linetype="dashed", alpha=.5) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Total Hours in Competitive Zip")
...
summary(lm(data = df.temp %>% filter(!is.na(Tx) & year.rel < 0) %>% mutate(Treated = ifelse(Att > 0, 1, 0)), Outcome ~ Treated*year.rel))
 ...
 df.comp %>% 
  ...
  add_p()
    ...
    df.z.dist %>% left_join(df.comp, by = c("BHCMIS ID.y" = "BHCMISID", "Year")) %>% 
      ...
      select(Type, Quality, Total, Altman) %>% 
          tbl_summary(by = Type, 
                statistic = list(all_continuous() ~ "{mean} ({sd})"), missing = "no") %>% 
  add_p()
