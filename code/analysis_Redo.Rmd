...
did2s(
      data = df.spill3 %>% mutate(clust = substr(Zip, 1, 3)), 
      yname = "Sites", 
      treatment = "Tx",
...
df.p %>% 
  ...
  #group_by(Zip) %>% 
  summarise(Radius = mean.nas(Radius), DistRivaltoCenter = mean.nas(DistRivaltoCenter))
...
df.p %>% 
  ...
  #group_by(Zip) %>% 
  summarise(Radius = mean.nas(Radius), DistIncumbtoCenter = mean.nas(DistIncumbtoCenter))
...
df.p %>% 
  ...
  #group_by(Zip) %>% 
  summarise(DistIncumbtoRival = mean.nas(DistIncumbtoRival))
