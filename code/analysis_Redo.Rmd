...
splits <- quantile(dfc[dfc$year.rel == -1,]$ngla_net_gain_loss / dfc[dfc$year.rel == -1,]$ngla_expense, c(0.25, 0.5, 0.75), na.rm = TRUE)
...
df.liquid <- df.liquid %>% 
  left_join(dfc %>% filter(Year == 2010) %>% select(ID, quick_ratio) %>% 
...
event_study(data = df.competitor, 
  yname = "Pct.None", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless) %>% 
  filter(abs(term) <= 7) %>% 
