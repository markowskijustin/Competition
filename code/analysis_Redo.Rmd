...
## Access by quantile
```{r}
qnats1 <- c(0, 0.2, 0.4, 0.6, 0.8)
qnats2 <- c(0.2, 0.4, 0.6, 0.8, 1)

qnats1 <- seq(0, 0.8, by = 0.2)
qnats2 <- seq(0.2, 1, by = 0.2)
 
   model.dist.quants <-  did2s(
      data = df.spill3 %>% filter(Dist > quantile(Dist, qnats1[1]) & Dist < quantile(Dist, qnats2[1])), 
      yname = "Total.Hours", 
      treatment = "Tx",
        first_stage = ~ 1 | Zip + Year , 
        second_stage = ~ i(Tx, ref = FALSE)  ,
  ...
  for (i in 2:5){
      model.dist.quants <-  did2s(
      data = df.spill3 %>% filter(Dist > quantile(Dist, qnats1[i]) & Dist < quantile(Dist, qnats2[i])), 
      yname = "Total.Hours", 
      treatment = "Tx",
        first_stage = ~ 1 | Zip + Year , 
        second_stage = ~ i(Tx, ref = FALSE)  ,
