...
df.profit <- df.profit %>% 
  ...
  mutate(profit.class = ifelse(ngla_net_gain_loss / ngla_revenue < 0.017, 1, 
              ifelse(ngla_net_gain_loss / ngla_revenue >= 0.017 & ngla_net_gain_loss / ngla_revenue <= 0.081, 2, 
              ifelse(ngla_net_gain_loss / ngla_revenue > 0.081, 3, NA)))) , by = c("ID")) 
...
for (g in 1:3){
  ...
  colnames(df.temp)[10] <- "Outcome"
  outcomes <- did2s(
      data = df.temp %>% filter(profit.class == g), 
      yname = "Outcome", 
      treatment = "Tx",
...
did2s(
      data = dfc.new %>% 
        inner_join(dfc %>% filter(Year == 2010 & ngla_net_gain_loss/ngla_revenue > 0.081 ) #& ngla_net_gain_loss/ngla_revenue < 0.03)  ngla_net_gain_loss/Assets < 0.05
                   %>% select(ID, Pct.NHA:Pct.COM) %>% unique() , by = c("ID")) , 
      yname = "Total", 
      treatment = "Tx",
        first_stage = ~ 0 | ID + Year + Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + Pct.MCD + Pct.MCR + Pct.None, 
