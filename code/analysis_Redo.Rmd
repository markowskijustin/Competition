...
zp3 <- event_study(data = df.spill3 %>% filter((Dist >= quantile(Dist, 0.9) & Att > 0) | Att == 0) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites)), 
...
```{r}
qnats1 <- seq(0, 0.833333, by = 0.166666)
qnats2 <- seq(0.16666, 1, by = 0.1666666)



 
   model.dist.quants <-  did2s(
      data = df.spill3 %>% filter((Dist == 0 & Att > 0) | Att == 0) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% mutate(Tx = ifelse(Att - 1 == Year, 1, Tx)), 
      yname = "NumberofPatients", 
      treatment = "Tx",
        first_stage = ~ 1 | Zip + Year , 
        second_stage = ~ i(Tx, ref = FALSE)  ,
  ...
  for (i in 1:6){
      model.dist.quants <-  did2s(
      data = df.spill3  %>% filter((Dist > quantile(Dist, qnats1[i]) & Dist <= quantile(Dist, qnats2[i]) & Att > 0 ) | Att == 0)%>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% mutate(Tx = ifelse(Att - 1 == Year, 1, Tx)) , 
      yname = "NumberofPatients", 
      treatment = "Tx",
        first_stage = ~ 1 | Zip + Year , 
        second_stage = ~ i(Tx, ref = FALSE)  ,
...
model <- did2s(
      data = df.spill3  %>% filter((Dist >= quantile(Dist, 0.9) & Att > 0) | Att == 0 ) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites), SL = log(Sites + 1)) %>% 
        mutate(Tx = ifelse(Att - 1 == Year, 1, Tx))  , 
      yname = "Total.Hours", 
      treatment = "Tx",
        first_stage = ~ 1 | Zip + Year , 
...
for (i in 11:19){
  df.temp <- df.spill3 %>% filter((Dist >= quantile(Dist, 0.9) & Att > 0) | Att == 0 )%>% 
        mutate(Tx = ifelse(Att - 1 == Year, 1, Tx))
...
sum(df.spill3 %>% filter(Att > 0 & Dist < 200 & Dist > 40 & Year == Att - 1) %>% select(Total.Hours))
