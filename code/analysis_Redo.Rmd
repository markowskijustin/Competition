...
df.select.stint <- dfc %>% 
  ...
  select(`BHCMIS ID`, value, Tx, Year, Att, Att.random, year.rel, ID, STATE, 
         Complexity, Intensity, #HIV.prev:Hypertension.care, pap.denom, crc.denom, adultbmi.denom, childbmi.denom, htn.denom, dm.denom,#MCD:COM, 
         Pct.NHA:Pct.COM)
...
p1 <- df.select.stint %>% 
  ...
  #           Quality = mean(Qcomp, na.rm = TRUE), 
  #           Finances = mean(Altman, na.rm = TRUE)) %>% 
  pivot_longer(cols = Complexity:Intensity) %>% 
  mutate(value = round(value, 2)) %>% 
  #filter(name %in% c("Total", "Qcomp", "Altman")) %>% 
  pivot_wider(id_cols = name, names_from = c(Treated, Post), values_from = value) %>% 
  mutate(Unadjusted = (`1_1` - `1_0`) - (`0_1` - `0_0`)) %>% 
...
df.spill3 <- df.spill2 %>% 
  left_join(census %>% select(ZipCode, YEAR, TotalPop, IncomeBelowPoverty, PovertyStatusDet), by = c("Year" = "YEAR", "Zip" = "ZipCode")) %>% 
