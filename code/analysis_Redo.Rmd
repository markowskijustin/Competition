...
for (j in 10:12){
  ...
  outcomes <- did2s(
      data = df.temp %>% 
        inner_join(df.core[,c(4, 8, 13:ncol(df.core))] %>% filter(Year == 2010 ) #& ngla_net_gain_loss/ngla_revenue < 0.03)  ngla_net_gain_loss/Assets < 0.05
...
splits <- quantile(dfc[dfc$Year == 2010,]$ngla_net_gain_loss / dfc[dfc$Year == 2010,]$ngla_expense, c(0.33, 0.66), na.rm = TRUE)
...
df.profit <- df.profit %>% 
  left_join(dfc %>% filter(Year == 2010) %>% select(ID, ngla_net_gain_loss, ngla_revenue, ngla_expense, Assets, Pct.NHA:Pct.COM) %>% 
  mutate(pc = ngla_net_gain_loss / ngla_expense) , by = c("ID")) %>% filter(`BHCMIS ID` %notin% c("010450","070430"))
  

df.profit <- df.profit %>% 
  mutate(profit.class = ifelse(pc < 0.017, 1, 
              ifelse(pc >= 0.017 & pc <= 0.081, 2, 
              ifelse(pc > 0.081, 3, NA))))
...
for (g in 1:3){
  ...
  outcomes <- did2s(
      ...
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ 0 | ID + Year + Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + Pct.MCR + Pct.None, 
        second_stage = ~ i(Tx, ref = FALSE),
        cluster_var = "ID",
      verbose = FALSE
    )
...
data.frame(Subset = c(rep("01Low", 3), rep("02Medium", 3), rep("03High", 3)),
...
splits <- quantile(dfc[dfc$Year == 2010,]$Total, c(0.5), na.rm = TRUE)
...
df.size <- df.size %>% 
  left_join(dfc %>% filter(Year == 2010) %>% select(ID, Total, Pct.NHA:Pct.COM) %>% 
  rename(Size = Total) , by = c("ID")) %>% 
  filter(`BHCMIS ID` %notin% c("010450","070430"))
  

splits1 <- c(min(df.size$Size, na.rm = TRUE), splits)
splits2 <- c(splits, max(df.size$Size, na.rm = TRUE)+1)
...
data.frame(Subset = c(rep("01Below", 3), rep("02Above", 3)),
      Outcome = rep(c("Total","Quality","Financial"), 2),
      Estimate = as.numeric(did.size[,1]), 
      SD = as.numeric(did.size[,2])) %>% 
  mutate(Subset.f = as.factor(Subset)) %>% 
  ggplot(aes(y = Subset.f, x = Estimate, xmin = Estimate - SD * 1.96, xmax = Estimate + SD * 1.96)) + 
  geom_point() + 
  xlab("Staggered 2-way DID Estimates") + 
ylab(NULL) + 
#adds the CIs
...
#theme(text=element_text(family="Times",size=12, color="black"))+
theme(panel.spacing = unit(1, "lines")) + 
  ggtitle("DID Estimates by Baseline Clinics Size")
```
