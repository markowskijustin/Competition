...
df.core <- dfc %>% 
  ...
  select(`BHCMIS ID`, value, Tx, Year, Att, Att.random, year.rel, ID, STATE, 
         Total,Qcomp, Altman, Sites, #MCD:COM, 
...
for (j in 10:13){
  ...
  outcomes <- did2s(
      ...
      yname = "Outcome", 
      treatment = "Tx",
        first_stage = ~ Pct.NHB + Pct.HL + Pct.LessThan100pctFPL | ID + Year , #Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.MCD + Pct.MCR + Pct.None | ID + Year , 
...
#splits <- quantile(dfc[dfc$Year == 2011,]$Total, c(0.5), na.rm = TRUE)
splits <- quantile(df.dist[df.dist$year.rel == -1,]$Dist.random, c(0.33, 0.66), na.rm = TRUE)
...
```{r}
df.spill3 <- df.spill2 %>% 
  left_join(census %>% select(ZipCode, YEAR, IncomeBelowPoverty, PovertyStatusDet), by = c("Year" = "YEAR", "Zip" = "ZipCode"))#%>% 
...
rownames(did.access.figures) <- NULL
 ...
 did.access.figures %>% 
  ...
  kable_styling(latex_options = c("striped")) %>%
  pack_rows(index = c("Patients" = 5, "Intensive Margin" = 2, "Extensive Margin" = 2)) %>% 
