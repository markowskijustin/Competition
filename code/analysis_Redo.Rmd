...
df.spill3 <- df.spill2 %>% filter(NumberofPatients > 50 ) %>% 
  ...
  mutate(Poverty = IncomeBelowPoverty / PovertyStatusDet) #%>% filter(NumberofPatients > 100 | NumberofPatients / TotalPop > 0.01)
...
zp3 <- event_study(data = df.spill3 %>% filter(Dist > quantile(Dist, 0.8)) %>% mutate(Hours = ifelse(Sites == 0, 0, Total.Hours / Sites)), 
  ...
  #facet_wrap(~V1, scales = "free_y", ncol = 3) + 
  theme_tufte_revised() + geom_vline(xintercept=-0.5, color="red", linetype="dashed", alpha=.5) + 
  geom_hline(yintercept=0, color="black", linetype="dashed", alpha=.5) + ggtitle("Hours per Week in Farthest 20% of Zips")+ xlab("Year Relative to Competitive Shock") + ylab("Hours per Week")
...
data.frame(rbind(did.core, did.select.stint, did.select.payer, did.rival)) %>% 
