...
df.sum <- df.qual.exp %>% #filter(`Grant Number` != "H80CS00225") %>% 
  ...
  select(`Grant Number`, Year, ID, Att, Sites, Qcomp, Altmans.Z, Pct.HTN.CNTRL) %>%
  ...
  select(Year, ZipFilter, ID, Att, Sites, Qcomp, Altmans.Z, Pct.HTN.CNTRL, Pct.NHA:Pct.Children, MCD, COM, MCR) %>% 
  mutate(Tx = ifelse(Year >= Att, 1, 0), 
         year.rel = Year - Att) %>% 
  unique() 
...
out.qual <- event_study(data = df.sum %>% filter(ZipFilter == 1), 
  yname = "Sites", idname = "ID", tname = "Year", gname = "Att", estimator = "all", xformla = ~Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + Pct.Children + MCD + MCR + COM)

out.qual %>% 
  filter(estimator %notin% c("Borusyak, Jaravel, Spiess (2021)","Callaway and Sant'Anna (2020)")) %>% 
filter(abs(term) <= 3) %>% 
