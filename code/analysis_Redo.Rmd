...
splits <- quantile(dfc[dfc$Year == 2010,]$ngla_net_gain_loss / dfc[dfc$Year == 2010,]$ngla_revenue, c(0.33, 0.66), na.rm = TRUE)
...
df.profit <- df.profit %>% 
  ...
  mutate(pc = ngla_net_gain_loss / ngla_revenue) , by = c("ID")) %>% 
  filter(`BHCMIS ID` %notin% c("010450","0438280","070430"))
...
splits1 <- c(min(df.profit$pc, na.rm = TRUE), splits)
...
data.frame(Subset = c(rep("Low", 3), rep("Medium", 3), rep("High", 3)),
...
did2s(
      data = dfc.new %>% 
        inner_join(dfc %>% filter(Year == 2010 & ngla_net_gain_loss/ngla_revenue > 0.08 ) #& ngla_net_gain_loss/ngla_revenue < 0.03)  ngla_net_gain_loss/Assets < 0.05
                   %>% select(ID, Pct.NHA:Pct.COM) %>% unique() , by = c("ID")) , 
      yname = "Total", 
      treatment = "Tx",
        first_stage = ~ 0 | ID + Year + Pct.NHA + Pct.NHB + Pct.HL + Pct.LessThan100pctFPL + Pct.FarmWorker + Pct.Homeless + Pct.MCD + Pct.MCR + Pct.None, 
